...
\begin{document}
   ...
   %% Don't write english letters in the abstract in korean.
    ...
    Collaborative filtering, widely adopted in recommendation systems, exploits the observed item ratings given by users to provide personalized recommendations under the assumption that all users honestly rate items.
    ...
    Recently, some researchers have been interested in the fact that most recommendation systems encourage users to write reviews, as well as rate the helpfulness of reviews written by other users based on review content.
    ...
    The proposed approach yields up to 20 times more robust recommendation results than the approaches that do not consider attacks.
...
\chapter{Conclusion}
% 추천 시스템은 사용자 경험이 훼손되지 않기 위해 다양한 조작 공격을 고려해야한다.
...
% 이전까지는 item rating을 조작하는 공격에 대한 방어법이 제안되어왔다. 
Most studies of robust recommendation systems have only considered attacks that manipulate item ratings.
% 하지만 이런 방법들은 item rating과 helpfulness rating 모두 조작하려는 공격에 대해 취약함을 보인다.
% However, these methods are vulnerable to attacks that manipulate both item rating and helpfulness rating
% 하지만 item rating과 helpfulness rating을 모두 조작하려는 공격은 이전에 제안된 방법을 무력화시킬수 있기 때문에, 이 학위 논문에선 이런 공격에 대해 견고한 추천 모델을 제안한다.
...
% User2Vec와 Bayesian을 제안하여 fake item rating의 영향력을 낮추고 이를 통해 이전보다 더 견고한 추천 결과를 도출한다.
...
% 실제 데이터에서의 실험 결과를 통해 우리 방법이 이전 방법보다 최대 20배 견고하다는 것을 보인다.
...
% 거짓으로 주입된 사용자를 알아내기 위한 User2Vec,Bayesian Weighted Mean 방법을 제안하여 fake helpfulness ratings이 fake review의 quality를 높이지 못하도록 막음으로써, 리뷰의 진실된 퀄리티를 추정한다.
...
\bibitem{TSNE} L. Van der Maaten and G. Hinton. Visualizing data using t-sne. Journal of Machine Learning Research, 9(2579-2605):85, 2008
